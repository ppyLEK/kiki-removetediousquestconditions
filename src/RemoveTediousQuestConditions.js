...
class RemoveTediousQuestConditions {
    ...
    postDBLoad(container) {
        ...
        const quests = this.container.resolve("DatabaseServer").getTables().templates.quests;
        ...
        conditionsToRemove.forEach(condition => {
            ...
            for (let eachQuest in quests)
                this.keyClearer(quests[eachQuest], condition, eachQuest, quests);
            if (this.config.debug === true && this.loggerBuffer.length > 0)
                this.logger.info([...new Set(this.loggerBuffer)]);
            this.loggerBuffer = [];
        });
    }
    /**
     ...
     */
    keyClearer(object, key, questId, quests) {
     ...
     * @param quests container/database/quests
     */
    findCounterCreators(target, quests) {
        if (this.config.debug === true)
            this.logger.info(`\n${target}`);
        for (let eachQuest in quests) {
     ...
     */
    removeNested(input, target, questName) {
        let thisCondition = input.counter.conditions;
        thisCondition.filter((finalCondition) => finalCondition.conditionType === target)
            .forEach((finalCondition) => {
...
//# sourceMappingURL=RemoveTediousQuestConditions.js.map